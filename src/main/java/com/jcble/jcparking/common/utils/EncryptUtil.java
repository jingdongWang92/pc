...
public abstract class EncryptUtil {
     ...
     * @throws Exception
     */
    public static String signature2(Map<String, String> params, String secret, boolean appendSecret, boolean isHMac,
            String signName) throws Exception {
        params.remove(signName);
        String[] names = params.keySet().toArray(ArrayUtils.EMPTY_STRING_ARRAY);
        Arrays.sort(names);
        ...
        for (int i = 0; i < names.length; i++) {
            String name = names[i];
            if (name.equals("account") || name.equals("mobile") || name.equals("otheraccount")
                    || name.equals("focusAccount")) {
                sb.append(name);
                sb.append(params.get(name).replaceAll(" ", "+"));
            } else {
                sb.append(name);
                sb.append(params.get(name));
            }
    ...
    public static void main(String[] args) throws Exception {
        ...
        paramsMap.put("user", "haha");
        paramsMap.put("password", "pwd");
        paramsMap.put("type", "web");
        String sign = signature(paramsMap, "xfb", true, false, "sign");
